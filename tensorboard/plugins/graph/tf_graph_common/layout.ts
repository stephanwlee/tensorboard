      ...
      case NodeType.META:
        if (!childNodeInfo.expanded) {
          ...
          childNodeInfo.height = PARAMS.nodeSize.meta.height(
            childNodeInfo.node.cardinality
          );
        } else {
...
function layoutAnnotation(renderNodeInfo: render.RenderNodeInfo): void {
  ...
  // and their hosts.
  let inTouchHeight = Math.min(
    renderNodeInfo.height / 2 - renderNodeInfo.radius,
    inboxHeight / 2
  );
  ...
  renderNodeInfo.height = Math.max(
    renderNodeInfo.height,
    inboxHeight,
    outboxHeight
  );
 ...
 */
export function computeCXPositionOfNodeShape(
  ...
  return (
    renderInfo.x - renderInfo.width / 2 + dx + renderInfo.coreBox.width / 2
  );
}
